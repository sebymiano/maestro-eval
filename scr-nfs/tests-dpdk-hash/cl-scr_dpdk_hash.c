...
typedef uint32_t map_key_hash_rte_hashmap(const void *data, __rte_unused uint32_t data_len, uint32_t init_val);
...
int expire_items_single_map_iteratively_rte_hash(struct Vector *vector, struct rte_hash *map,
                                        int *map_values, int start, int n_elems) {
  assert(start >= 0);
  assert(n_elems >= 0);
  void *key;
  for (int i = start; i < n_elems; i++) {
    vector_borrow(vector, i, (void **)&key);
    map_erase_rte_hash(map, map_values, key, sizeof(key), (void **)&key);
    vector_return(vector, i, key);
  }
}
...
int sketch_allocate(map_key_hash *kh, map_key_hash_rte_hashmap *kh_rte, int key_len, uint32_t capacity, uint16_t threshold,
  ...
  snprintf(map_name, sizeof(map_name), "sketch_map_lcore%d", rte_lcore_id());

  struct rte_hash_parameters map_allocation_params = {
		.name = map_name,
		.entries = total_sketch_capacity,
		.key_len = key_len,
		.hash_func = kh_rte,
		.hash_func_init_val = 0,
	};

  (*sketch_out)->key_len = key_len;

  printf("Allocating rte_hash map on lcore: %u\n", rte_lcore_id());
  struct rte_hash *map = rte_hash_create(&map_allocation_params);
  if (map == NULL) {
    printf("Failed to allocate rte_hash map on sketch lcore: %u\n", rte_lcore_id());
    /* Print also errno */
    printf("rte_errno: %d\n", rte_errno);
    return 0;
  }
  (*sketch_out)->clients = map;

  void *map_values_ptr = NULL;
  /* Now allocate the memory for the array holding the values */
  map_values_ptr = rte_malloc(NULL, total_sketch_capacity * sizeof(int), 64);
  if (map_values_ptr == NULL) {
    printf("Failed to allocate values array on lcore: %u\n", rte_lcore_id());
    return 0;
  } else {
    printf("Allocated values array on lcore: %u\n", rte_lcore_id());
    (*sketch_out)->clients_values = map_values_ptr;
  }
...
void sketch_refresh(struct Sketch *sketch, vigor_time_t now) {
  for (int i = 0; i < SKETCH_HASHES; i++) {
    map_get_rte_hash(sketch->clients, sketch->clients_values, &sketch->internal.hashes[i],
...
int sketch_touch_buckets(struct Sketch *sketch, vigor_time_t now) {
  for (int i = 0; i < SKETCH_HASHES; i++) {
    int bucket_index = -1;
    int present =
        map_get_rte_hash(sketch->clients, sketch->clients_values, &sketch->internal.hashes[i], &bucket_index);
    ...
    if (!present) {
      ...
      (*saved_bucket) = 0;
      map_put_rte_hash(sketch->clients, sketch->clients_values, saved_hash, bucket_index);
...
void sketch_expire(struct Sketch *sketch, vigor_time_t time) {
  ...
  for (int i = 0; i < SKETCH_HASHES; i++) {
    ...
    while (dchain_expire_one_index(sketch->allocators[i], &index, time)) {
      ...
      map_erase_rte_hash(sketch->clients, sketch->clients_values, key, sketch->key_len, &key);
    ...
    for (uint16_t VIGOR_DEVICE = 0; VIGOR_DEVICE < VIGOR_DEVICES_COUNT;
      ...
      for (uint16_t n = 0; n < rx_count; n++) {
        ...
        for (int i = 0; i < NUM_CORES - 1; i++) {
          ...
          md->timestamp = VIGOR_NOW - 10 * (NUM_CORES - i) - 10;
...
uint32_t client_hash_rte_hashmap(const void *data, __rte_unused uint32_t data_len, uint32_t init_val) {
  struct client *id = (struct client *)data;
...
{
	struct flow *id = (struct flow *)data;
  ...
  if (map_allocation_succeeded__1) {
    ...
    if (vector_alloc_success__4) {
      ...
      if (is_dchain_allocated__7) {
        int sketch_allocation_succeeded__10 = sketch_allocate(client_hash, client_hash_rte_hashmap, sizeof(struct client), spread_data_among_cores(1024u), 64u, &(*sketch_ptr));
...
int nf_process_scr(uint16_t device, struct metadata_elem *state_elem, int64_t now) {
  ...
  if ((8u == state_elem->ether_type) & (20ul <= (4294967282u + state_elem->packet_len))) {
    if (((6u == state_elem->protocol) | (17u == state_elem->protocol)) & ((4294967262u + state_elem->packet_len) >= 4ul)) {
      int number_of_freed_flows__36 = expire_items_single_map_rte_hash((*dchain_ptr), (*vector_ptr), (*map_ptr), (*map_values_ptr), now - 100000000000ul);
      ...
      if (0u != device) {
        ...
        int map_has_this_key__48 = map_get_rte_hash((*map_ptr), (*map_values_ptr), map_key, &map_value_out);
        ...
        if (0u == map_has_this_key__48) {
          ...
          if (false == ((out_of_space__52) & (0u == number_of_freed_flows__36))) {
            ...
            map_put_rte_hash((*map_ptr), (*map_values_ptr), map_key, new_index__52);
