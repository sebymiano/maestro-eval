...
bool nf_init(void);
int nf_process(struct Map** map_ptr, struct Vector** vector_ptr, struct DoubleChain** dchain_ptr, uint16_t device, uint8_t *buffer, uint16_t packet_length,
               vigor_time_t now);
int nf_process_scr(struct Map** map_ptr, struct Vector** vector_ptr, struct DoubleChain** dchain_ptr, uint16_t device, struct metadata_elem *state_elem, int64_t now);
        ...
        for (uint16_t m = n; m < n + PKT_PREFETCH_DISTANCE && m < rx_count; m++) {
          ...
          for (int i = 0; i < NUM_CORES - 1; i += MD_PREFETCH_DISTANCE) {
            ...
            for (int j = i; j < i + MD_PREFETCH_DISTANCE && j < NUM_CORES - 1; j++) {
              ...
              nf_process_scr(map_ptr, vector_ptr, dchain_ptr, mbufs[m]->port, md, md->timestamp);
          ...
          uint16_t dst_device = nf_process(map_ptr, vector_ptr, dchain_ptr, mbufs[m]->port, current_pkt_data, mbufs[m]->pkt_len, VIGOR_NOW);
...
uint8_t hash_key_0[RSS_HASH_KEY_LENGTH] = {
  ...
  0x50, 0x2e, 0x26, 0x8b, 0x52, 0xc2, 0x2c, 0xbc, 
  0xdc, 0x9c, 0x8b, 0x18, 0x8, 0x29, 0xc, 0xfe, 
  0xd6, 0xa1, 0x29, 0x9e, 0x41, 0x9, 0xaa, 0x91
};
uint8_t hash_key_1[RSS_HASH_KEY_LENGTH] = {
  0x0, 0x0, 0x0, 0x1, 0x0, 0x0, 0x0, 0x0, 
  0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x1, 
  0x3, 0x21, 0xd8, 0x4b, 0x2a, 0x13, 0x98, 0x75, 
  0xf1, 0x95, 0xd5, 0xba, 0x2f, 0x8e, 0x4b, 0x28, 
  0xe2, 0xec, 0xb9, 0x19, 0xd1, 0x7c, 0xc9, 0xe0
};
...
int nf_process_scr(struct Map** map_ptr, struct Vector** vector_ptr, struct DoubleChain** dchain_ptr, uint16_t device, struct metadata_elem *state_elem, int64_t now) {
  ...
  if ((8u == state_elem->ether_type) & (20ul <= (4294967282u + state_elem->packet_len))) {
    ...
    if (((6u == state_elem->protocol) | (17u == state_elem->protocol)) & ((4294967262u + state_elem->packet_len) >= 4ul)) {
      ...
      // 127
      if (0u != device) {
        int32_t dchain_is_index_allocated__36 = dchain_is_index_allocated((*dchain_ptr), state_elem->dst_port);

        // 125
        // 126
        if (dchain_is_index_allocated__36) {
          uint8_t* vector_value_out = 0u;
          vector_borrow((*vector_ptr), state_elem->dst_port, (void**)(&vector_value_out));
          vector_return((*vector_ptr), state_elem->dst_port, vector_value_out);

          // 125
          if (((vector_value_out[8ul] == state_elem->src_addr) & (vector_value_out[2ul] == state_elem->src_port)) & (vector_value_out[14ul] == state_elem->protocol)) {
            /* No need to do that, we are computing for SCR here */
      ...
      // 130
        ...
        map_key[14u] = state_elem->protocol;
        int map_value_out;
        ...
        if (0u == map_has_this_key__68) {
          ...
          if (false == ((out_of_space__71))) {
            ...
            vector_value_out[14u] = state_elem->protocol;
            ...
            // ether_header_1->d_addr.addr_bytes[5] = 0xf7;
            ...
            // ether_header_1->s_addr.addr_bytes[4] = 0x04;
            // ether_header_1->s_addr.addr_bytes[5] = 0x83;
            return 1;
          }
...
int nf_process(struct Map** map_ptr, struct Vector** vector_ptr, struct DoubleChain** dchain_ptr, uint16_t device, uint8_t *packet, uint16_t packet_length, vigor_time_t now) {
  // int number_of_freed_flows__24 = expire_items_single_map((*dchain_ptr), (*vector_ptr), (*map_ptr), now - 100000000000ul);
      ...
      // 130
        ...
        if (0u == map_has_this_key__68) {
          ...
          if (false == ((out_of_space__71))) {
            ...
            ether_header_1->dst_addr.addr_bytes[5] = 0xf7;
            ...
            ether_header_1->src_addr.addr_bytes[4] = 0x04;
            ether_header_1->src_addr.addr_bytes[5] = 0x83;
            #else
            ether_header_1->d_addr.addr_bytes[0] = 0xb8;
            ether_header_1->d_addr.addr_bytes[1] = 0x3f;
            ether_header_1->d_addr.addr_bytes[2] = 0xd2;
