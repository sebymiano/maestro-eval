...
struct metadata_elem {
  uint8_t d_addr[RTE_ETHER_ADDR_LEN]; /**< Addr bytes in tx order */
  uint64_t timestamp;
...
void print_md(uint16_t device, uint16_t lcore_id, struct metadata_elem *md) {
  ...
  printf("  - Dst MAC: %02X:%02X:%02X:%02X:%02X:%02X\n",
         md->d_addr[0], md->d_addr[1], md->d_addr[2],
         md->d_addr[3], md->d_addr[4], md->d_addr[5]);
  printf("\n");
}
...
bool nf_init() {
  ...
  struct Vector** vector_2_ptr = &RTE_PER_LCORE(_vector_2);
  struct DoubleChain** dchain_ptr = &RTE_PER_LCORE(_dchain);
  int map_allocation_succeeded__1 = map_allocate(rte_ether_addr_eq, rte_ether_addr_hash, spread_data_among_cores(65535u), &(*map_ptr));
  ...
  if (map_allocation_succeeded__1) {
    int vector_alloc_success__4 = vector_allocate(6u, spread_data_among_cores(65535u), rte_ether_addr_allocate, &(*vector_ptr));
    ...
    if (vector_alloc_success__4) {
      int vector_alloc_success__7 = vector_allocate(2u, spread_data_among_cores(65535u), DynamicValue_allocate, &(*vector_1_ptr));
      ...
      if (vector_alloc_success__7) {
        ...
        if (map_allocation_succeeded__10) {
          ...
          if (vector_alloc_success__13) {
            int is_dchain_allocated__16 = dchain_allocate(spread_data_among_cores(65535u), &(*dchain_ptr));
...
int nf_process_scr(struct Map** map_ptr, struct Vector** vector_ptr, struct Map** map_1_ptr, struct Vector** vector_1_ptr, struct Vector** vector_2_ptr, struct DoubleChain** dchain_ptr, uint16_t device, struct metadata_elem* state_elem, int64_t now) {
  ...
  map_key[2u] = state_elem->d_addr[2ul];
  map_key[3u] = state_elem->d_addr[3ul];
  map_key[4u] = state_elem->d_addr[4ul];
  map_key[5u] = state_elem->d_addr[5ul];
