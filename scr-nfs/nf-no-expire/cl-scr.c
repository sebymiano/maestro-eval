...
bool nf_init(void);
int nf_process(struct Map** map_ptr, struct Vector** vector_ptr, struct DoubleChain** dchain_ptr, struct Sketch** sketch_ptr, uint16_t device, uint8_t *buffer, uint16_t packet_length,
               vigor_time_t now);
int nf_process_scr(struct Map** map_ptr, struct Vector** vector_ptr, struct DoubleChain** dchain_ptr, struct Sketch** sketch_ptr, uint16_t device, struct metadata_elem *state_elem, int64_t now);
        ...
        for (uint16_t m = n; m < n + PKT_PREFETCH_DISTANCE && m < rx_count; m++) {
          ...
          for (int i = 0; i < NUM_CORES - 1; i += MD_PREFETCH_DISTANCE) {
            ...
            for (int j = i; j < i + MD_PREFETCH_DISTANCE && j < NUM_CORES - 1; j++) {
              ...
              nf_process_scr(map_ptr, vector_ptr, dchain_ptr, sketch_ptr, mbufs[m]->port, md, md->timestamp);
          ...
          uint16_t dst_device = nf_process(map_ptr, vector_ptr, dchain_ptr, sketch_ptr, mbufs[m]->port, current_pkt_data, mbufs[m]->pkt_len, VIGOR_NOW);
...
bool nf_init() {
  ...
  struct DoubleChain** dchain_ptr = &RTE_PER_LCORE(_dchain);
  struct Sketch** sketch_ptr = &RTE_PER_LCORE(_sketch);
  int map_allocation_succeeded__1 = map_allocate(flow_eq, flow_hash, spread_data_among_cores(65535u), &(*map_ptr));
  ...
  if (map_allocation_succeeded__1) {
    int vector_alloc_success__4 = vector_allocate(13u, spread_data_among_cores(65535u), flow_allocate, &(*vector_ptr));
...
int nf_process(struct Map** map_ptr, struct Vector** vector_ptr, struct DoubleChain** dchain_ptr, struct Sketch** sketch_ptr, uint16_t device, uint8_t* packet, uint16_t packet_length, int64_t now) {
