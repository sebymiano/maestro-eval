...
def gen_pcap_with_md_sbridge(num_cores, src_mac, dst_mac, output_path, input_file, pkt_len, scr_on, overwrite=False, quiet=False):
    print(f"[gen_pcap_with_md_sbridge] start num_cores: {num_cores}")

    if not os.path.exists(output_path):
        os.makedirs(output_path)

    output_file = f"{output_path}/dpdk_sbridge_scr_{num_cores}cores.pcap"

    if os.path.exists(output_file):
        if overwrite:  # If overwrite is enabled, delete the existing file
            os.remove(output_file)
        else:
            print(f"[gen_pcap_with_md_sbridge] Output file {output_file} already exists. Exiting.")
            return
    append_flag = False
    ...
    print(f"[gen_pcap_with_md_sbridge] Total packets in {input_file}: {total_packets}")
    ...
    with PcapWriter(output_file, linktype=DLT_EN10MB) as pkt_wr:
        for i, curr_pkt in read_packets(input_file):
            ...
            print(f"\r[gen_pcap_with_md_sbridge] Generating {output_file} ({100 * (i+1) / total_packets:3.2f} %) ...", end="")

    print("")
    print(f"[gen_pcap_with_md_sbridge] output pcap: {output_file}")
    print("[gen_pcap_with_md_sbridge] Done!")
