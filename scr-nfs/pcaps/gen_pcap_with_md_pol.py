...
def gen_pcap_with_md_pol(num_cores, src_mac, dst_mac, output_path, input_file, pkt_len, scr_on, overwrite=False, quiet=False):
    print(f"[gen_pcap_with_md_pol] start num_cores: {num_cores}")

    if not os.path.exists(output_path):
        os.makedirs(output_path)

    output_file = f"{output_path}/dpdk_pol_scr_{num_cores}cores.pcap"

    if os.path.exists(output_file):
        if overwrite:  # If overwrite is enabled, delete the existing file
            os.remove(output_file)
        else:
            print(f"[gen_pcap_with_md_pol] Output file {output_file} already exists. Exiting.")
            return
    append_flag = False
    ...
    print(f"[gen_pcap_with_md_pol] Total packets in {input_file}: {total_packets}")
    ...
    print("[gen_pcap_with_md_pol] Done!")
