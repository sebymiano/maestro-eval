...
static int create_drop_filter(uint16_t port_id) {
  ...
  rte_memcpy(&eth_spec.dst.addr_bytes, mac, RTE_ETHER_ADDR_LEN);
//   memset(&eth_mask.dst.addr_bytes, 0xFF, RTE_ETHER_ADDR_LEN - 1);  // Full match on the source MAC
...
static int create_mac_filter(uint16_t port_id, struct mac_to_queue_map *mac_map, size_t mac_map_size) {
    ...
    for (size_t i = 0; i < mac_map_size; i++) {
        ...
        // Specify the source MAC address to match
        rte_memcpy(&eth_spec.dst.addr_bytes, mac_map[i].mac, RTE_ETHER_ADDR_LEN);
        memset(&eth_mask.dst.addr_bytes, 0xFF, RTE_ETHER_ADDR_LEN);  // Full match on the source MAC
