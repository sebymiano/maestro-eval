...
if __name__ == "__main__":
	...
	parser.add_argument('--flows', help='number of unique flows (> 0)', type=int, default=-1, required=False)
	...
	generate_unique_flows = True

	if args.pcap != None:
		assert(Path(args.pcap).exists())
		unique_flows, flows, pkt_sizes = utils.read_trace(args.pcap, args.max)
		if args.flows > 0:
			if args.flows > len(unique_flows):
				print(f"Warning: {args.flows} flows requested, but only {len(unique_flows)} unique flows found in {args.pcap}.")
			print(f"We are generating {args.flows} flows to accomodate the request.")
