    ...
    for workload in workload_types:
        ...
        for app in applications:
            for tech in technologies:
                ...
                filename = os.path.join(data_dir, f"{app.lower()}-{tech.lower().replace(' ', '-')}-{workload}_{graph_type}.dat")
        ...
        fig.suptitle(f'Technologies Comparison {graph_type.capitalize()} - {workload.capitalize()}', fontsize=20, y=0.91)
        ...
        output_path = os.path.join(output_dir, f'technologies-comparison-{workload}-{graph_type}.pdf')
